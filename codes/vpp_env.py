...
class vpp_env:
    ...
    def step(self,action):
        ...
        for i,ev in enumerate(self.charging_EVs):
            ...
            if self.soc[ev]<=0.1 and (not action[i]>0):
                return None,None,None
            elif 0.1<=self.soc[ev] and self.soc[ev] <=0.2 and action[i]<0:
                return None,None,None
        ...
        states=np.zeros(2+self.num_EV_charger)
        states[0]=action.T@np.ones_like(action)
        power=exist_energy-states[0]
        states[1]=power
        states[2:]=self.soc[self.charging_EVs]
        ...
        if not self.t==0 and self.t%900==0:
            pd.DataFrame(self.soc).to_csv(f'../figurs/SOC_{self.name}.csv')
            print(self.soc)
            try:
    ...
    def trajectory_ending_reward(self,soc,greed_energy,renewable_energy,cost):
        ...
        if greed_energy<800:
            rewards[1]=-10*greed_energy+8000
        else:
            rewards[1]=-greed_energy+800
            
        if renewable_energy<250:
            rewards[2]=-5*renewable_energy/3+700
        else:
            rewards[2]=-3*renewable_energy/2+375
        print('Exceed power ',renewable_energy,'purched power ',greed_energy,'Cost ',cost)
        if cost<200:
            rewards[3]=-40*cost+8000
        else:
            rewards[3]=-10*cost+2000
